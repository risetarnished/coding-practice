...
public class BinaryTreePathSumToTargetIII {
  ...
  private boolean existPath(TreeNode root, Set<Integer> prefixSums,
    ...
    if (root.left != null && existPath(root.left, prefixSums, currentSum, target)) {
      return true;
    }
    if (root.right != null && existPath(root.right, prefixSums, currentSum, target)) {
      return true;
    }
    if (needRemove) {
